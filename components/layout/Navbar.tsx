    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/60 bg-background/85 backdrop-blur-xl">
      <div className="mx-auto flex h-20 max-w-8xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden md:block shrink-0">
          <Link
           	href="/contact"
            className="inline-flex items-center justify-center rounded-full bg-primary px-6 py-2.5 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-red)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_12px_40px_-8px_oklch(0.55_0.22_25_/_0.35)]"
          >
          ...
          <div className="flex flex-col gap-2">
            {navLinks.map((l) =>
              l.label === "Services" ? (
                ...
                <MobileAboutMenu key={l.to} onClose={() => setOpen(false)} />
              ) : (
                <Link
                  key={l.to}
                 	href={l.to}
                  onClick={() => setOpen(false)}
                  className={`rounded-md px-4 py-3 text-sm font-medium transition-colors duration-300 ${pathname === l.to || pathname.startsWith(l.to + "/") ? "text-primary" : "text-foreground/80 hover:text-primary hover:bg-muted"}`}
                >
                  {l.label}
                </Link>
              )
            ...
            <Link
             	href="/contact"
              onClick={() => setOpen(false)}
              className="mt-4 inline-flex items-center justify-center rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-red)]"
            >
              Book Free Consultation
            </Link>
          </div>
